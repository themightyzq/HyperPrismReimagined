...
HighPassEditor::HighPassEditor(HighPassProcessor& p)
    : AudioProcessorEditor(&p), audioProcessor(p),
      presetBar(p.getValueTreeState(), "HyperPrism Reimagined High-Pass Filter", "HighPass")
{
    ...
    // Title
    titleLabel.setText("HIGH-PASS FILTER", juce::dontSendNotification);
    ...
    frequencySlider.getProperties().set("xyParamID", HighPassProcessor::FREQUENCY_ID);
    resonanceSlider.addMouseListener(this, true);
    resonanceSlider.getProperties().set("xyParamID", HighPassProcessor::RESONANCE_ID);
    gainSlider.addMouseListener(this, true);
    gainSlider.getProperties().set("xyParamID", HighPassProcessor::GAIN_ID);
    mixSlider.addMouseListener(this, true);
    mixSlider.getProperties().set("xyParamID", HighPassProcessor::MIX_ID);
    ...
    // Tooltips
    frequencySlider.setTooltip("Frequency below which audio is attenuated");
    frequencySlider.setDescription("Frequency below which audio is attenuated");
...
void HighPassEditor::paintOverChildren(juce::Graphics& g)
{
...
void HighPassEditor::updateParameterColors()
{
    ...
    updateSliderXY(frequencySlider, HighPassProcessor::FREQUENCY_ID);
    updateSliderXY(resonanceSlider, HighPassProcessor::RESONANCE_ID);
    updateSliderXY(gainSlider, HighPassProcessor::GAIN_ID);
    updateSliderXY(mixSlider, HighPassProcessor::MIX_ID);
    repaint();
}
...
void HighPassEditor::mouseDown(const juce::MouseEvent& event)
{
...
void HighPassEditor::showParameterMenu(juce::Component* target, const juce::String& parameterID)
{
    juce::PopupMenu menu;
