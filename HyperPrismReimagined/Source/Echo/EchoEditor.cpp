...
EchoEditor::EchoEditor(EchoProcessor& p)
    : AudioProcessorEditor(&p), audioProcessor(p),
      presetBar(p.getValueTreeState(), "HyperPrism Reimagined Echo", "Echo")
{
    ...
    // Title
    titleLabel.setText("ECHO", juce::dontSendNotification);
    ...
    delaySlider.getProperties().set("xyParamID", EchoProcessor::DELAY_ID);
    feedbackSlider.addMouseListener(this, true);
    feedbackSlider.getProperties().set("xyParamID", EchoProcessor::FEEDBACK_ID);
    mixSlider.addMouseListener(this, true);
    mixSlider.getProperties().set("xyParamID", EchoProcessor::MIX_ID);
    ...
    // Tooltips
    delaySlider.setTooltip("Time between echoes in milliseconds");
    delaySlider.setDescription("Time between echoes in milliseconds");
    feedbackSlider.setTooltip("Number of echo repeats -- higher values create longer trails");
    feedbackSlider.setDescription("Number of echo repeats -- higher values create longer trails");
    mixSlider.setTooltip("Balance between dry signal and echo");
    mixSlider.setDescription("Balance between dry signal and echo");
...
void EchoEditor::paintOverChildren(juce::Graphics& g)
{
...
void EchoEditor::paint(juce::Graphics& g)
{
    ...
    paintColumnHeader(delaySlider.getX() - 2, delaySlider.getY() - 20, 200,
...
void EchoEditor::resized()
{
    ...
    // Column 1: TIMING — Delay, Feedback
    int y1 = colTop + knobDiam / 2;
    centerKnob(delaySlider, delayLabel, col1.getX(), colWidth, y1, knobDiam);
...
void EchoEditor::updateParameterColors()
{
    ...
    updateSliderXY(delaySlider, EchoProcessor::DELAY_ID);
    updateSliderXY(feedbackSlider, EchoProcessor::FEEDBACK_ID);
    updateSliderXY(mixSlider, EchoProcessor::MIX_ID);
    repaint();
}
...
void EchoEditor::mouseDown(const juce::MouseEvent& event)
{
...
void EchoEditor::showParameterMenu(juce::Component* target, const juce::String& parameterID)
{
    juce::PopupMenu menu;
