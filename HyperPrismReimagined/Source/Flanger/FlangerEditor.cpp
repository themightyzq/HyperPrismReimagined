    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    titleLabel.setText("HyperPrism Reimagined Flanger", juce::dontSendNotification);
...
void FlangerEditor::resized()
{
    ...
    // Second row: Phase, Mix, Low Cut, High Cut
    auto secondRow = bounds.removeFromTop(140);
    secondRow.removeFromLeft(startX);
    
    phaseSlider.setBounds(secondRow.removeFromLeft(sliderWidth).reduced(0, 10));
...
void FlangerEditor::updateParameterColors()
{
    ...
    updateLabelColor(rateLabel, FlangerProcessor::RATE_ID);
    updateLabelColor(depthLabel, FlangerProcessor::DEPTH_ID);
    updateLabelColor(feedbackLabel, FlangerProcessor::FEEDBACK_ID);
    updateLabelColor(delayLabel, FlangerProcessor::DELAY_ID);
    updateLabelColor(phaseLabel, FlangerProcessor::PHASE_ID);
    updateLabelColor(mixLabel, FlangerProcessor::MIX_ID);
    updateLabelColor(lowCutLabel, FlangerProcessor::LOW_CUT_ID);
    updateLabelColor(highCutLabel, FlangerProcessor::HIGH_CUT_ID);
}
...
void FlangerEditor::showParameterMenu(juce::Label* label, const juce::String& parameterID)
{
    juce::PopupMenu menu;
